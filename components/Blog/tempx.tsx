...
function BlogSearch({posts} : BlogSearchProps) {
    ...
    const filteredPosts = searchTerm ?
        posts.filter((post) => post.title.toLowerCase().includes(searchTerm.toLowerCase())) : posts;
      ...
      <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 ">
        {currentPosts.length > 0 ? (
          currentPosts.map((post) => (
            <li key={post._id} className="bg-white shadow-md rounded-lg overflow-hidden p-4 flex flex-col justify-between space-y-4">
